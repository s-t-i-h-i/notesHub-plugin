...
console.log('\n--- bypasses found in review, each one refused now ---');
{
	...
	check('raw <code> is refused whatever it holds', kinds('a.md', 'T: <code>&#36;= app.foo()</code>').join() === 'raw-code,entity');
	check('a fence survives Windows line endings', kinds('a.md', '```dataviewjs\r\nx\r\n```\r\n').join() === 'fence:dataviewjs');
	check('a quoted fence hides nothing below it', kinds('a.md', '> ```\n\n```dataviewjs\nx\n```').join() === 'fence:dataviewjs');
	check('a callout keeps its own fence closed', kinds('a.md', '> ```text\n> a\n>\n> b\n> ```').join() === '');
